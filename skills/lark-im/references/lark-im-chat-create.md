...
# Create a topic chat; use --type public if discoverability is required
lark-cli im +chat-create --name "Topic Group" --chat-mode topic --type public
...
| `--chat-mode <mode>` | No | `group` (default) or `topic` | Group mode; `topic` creates a topic chat (not the same as `group_message_type=thread`) |
